...
<body class="min-h-screen flex items-center justify-center bg-gradient-to-r from-blue-400 to-pink-400 dark:bg-gray-900">
    <div class="container mx-auto max-w-xs sm:max-w-md md:max-w-lg lg:max-w-xl shadow-md dark:shadow-white py-4 px-6 sm:px-10 bg-white bg-gradient-to-r from-blue-600 to-pink-600 border-emerald-500 rounded-md">
        ...
        <div class="my-3">
            <h1 class="text-center text-2xl sm:text-3xl font-bold text-gray-900 dark:text-white">Edit Supply</h1>
            <form id="editSupplierForm" action="/api/itemsupplier/edit/{{ $itemsupplier->id }}" method="POST">
                @csrf
                @method('PUT')
                ...
                <div class="my-2">
                    ...
                    <input type="text" name="itemname" id="itemname" value="{{ $itemsupplier->item->name }}" class="block w-full border border-emerald-500 outline-emerald-800 px-2 py-2 text-sm sm:text-md rounded-md my-2 bg-white text-gray-900 dark:text-gray-900">
                </div>
                <div class="my-2">
                    <label for="suppliername" class="text-sm sm:text-md font-bold text-gray-700 dark:text-gray-300">Supplier Name</label>
                    <input type="text" name="suppliername" id="suppliername" value="{{ $itemsupplier->supplier->name }}" class="block w-full border border-emerald-500 outline-emerald-800 px-2 py-2 text-sm sm:text-md rounded-md my-2 bg-white text-gray-900 dark:text-gray-900">
                </div>
                <div class="my-2">
                    <label for="buyprice" class="text-sm sm:text-md font-bold text-gray-700 dark:text-gray-300">Buy Price</label>
                    <input type="number" name="buyprice" id="buyprice" step="0.01" min="0" value="{{ $itemsupplier->buyprice }}" class="block w-full border border-emerald-500 outline-emerald-800 px-2 py-2 text-sm sm:text-md rounded-md my-2 bg-white text-gray-900 dark:text-gray-900">
                </div>
                <div class="my-2">
                    <label for="quantity" class="text-sm sm:text-md font-bold text-gray-700 dark:text-gray-300">Quantity</label>
                    <input type="number" name="quantity" id="quantity" min="0" value="{{ $itemsupplier->quantity }}" class="block w-full border border-emerald-500 outline-emerald-800 px-2 py-2 text-sm sm:text-md rounded-md my-2 bg-white text-gray-900 dark:text-gray-900">
                </div>
